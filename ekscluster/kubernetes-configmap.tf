...
locals {
  configmap_roles = [
    ...
    },
    {
      ...
      username = "user1" # Just a place holder name
  ...
  configmap_users = [
    for key, user in aws_iam_user.admin_user :
    {
      userarn  = user.arn
      username = user.name
      groups   = ["system:masters"]
  ...
  #   # },
  #   {
  #     userarn  = "${aws_iam_user.admin_user[each.key].arn}"
  #     username = "${aws_iam_user.admin_user[each.key].name}"
  #     groups   = ["system:masters"]
  #   }  
  # ]   
}
